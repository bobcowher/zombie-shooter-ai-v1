...
ssh root@${RUNPOD_HOST} -p ${RUNPOD_PORT} "cd /app/${RUNPOD_PROJECT_NAME} && pip install -r requirements.txt"
ssh root@${RUNPOD_HOST} -p ${RUNPOD_PORT} "cd /app/${RUNPOD_PROJECT_NAME} && nohup bash -c 'python train.py > train.log 2>&1 &'"
ssh root@${RUNPOD_HOST} -p ${RUNPOD_PORT} "cd /app/${RUNPOD_PROJECT_NAME} && nohup bash -c 'tensorboard --logdir=\"./runs\" --port=6007 > tensorboard.log 2>&1 &'"
