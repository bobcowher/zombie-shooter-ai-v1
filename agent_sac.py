...
class Agent(object):
        ...
        for episode in range(episodes):
            ...
            while not done and episode_steps < max_episode_steps:
                ...
                if self.memory.can_sample(batch_size):
                    qf1_loss, policy_loss, alpha_loss = self.update_parameters(batch_size=batch_size, updates=total_steps)
    ...
    def update_parameters(self, batch_size, updates):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch = self.memory.sample_buffer(batch_size=batch_size)
        ...
        with torch.no_grad():
            # Target Q-values for the next state
            next_q_values = self.critic_target(next_state_batch)
            next_q_value = torch.max(next_q_values, dim=1, keepdim=True)[0]
            q_target = reward_batch + mask_batch * self.gamma * next_q_value
